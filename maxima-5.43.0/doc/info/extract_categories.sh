...
for f in *.texi; do
  if [ $f = "maxima.texi" ]
    then echo SKIP OVER $f
      ...
      /usr/bin/sed 's/^@deffn  *{[^}]*}  *\([^[:blank:]]*\).*/@anchor{Item: \1}\
...
cat *.texi\
  ...
  | /usr/bin/sed 's/\$/---endofline---/'\
  | /usr/bin/sed -f "$d/extract_categories1.sed" \
...
/usr/bin/sed 's/^@bye//' $TARGET_TEXI > tmp-target.texi
...
do
    ...
    if [ $? = 0 ]; then
        ...
        /usr/bin/sed 's/^&middot;$//; s/<p>\(<a href=".*">Category: .*<\/a>\)/<p>Categories:\&nbsp;\&nbsp;\1/' $f > tmp.html
        mv tmp.html $f
    fi
done
...
do
    ...
    if [ $? = 0 ]; then
        ...
        /usr/bin/sed 's/<a href="\(.*\)">Category: \(.*\)<\/a>/<a href="\1">\2<\/a>/' $f > tmp.html
        mv tmp.html $f
    fi
done
...
do
    ...
    if [ $? = 0 ]; then
        ...
        /usr/bin/sed 's/<a href="\(.*\)">Item: \(.*\)<\/a>/<a href="\1">\2<\/a>/' $f > tmp.html
        mv tmp.html $f
    fi
done
