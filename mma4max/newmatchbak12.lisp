...
(defun m1 (p e name gh c)
  ...
  (let* ((hp (Head p))
    ...
    (cond 
     ...
     ((and (atom e)(member hp '(Rational Complex) :test 'eq)(or (rationalp e)(complexp e)))
	      (cond ((rationalp e) (m1 p `(Rational ,(numerator e),(denominator e)) name gh c))
		    ((complexp e) (m1 p `(Complex ,(realpart e),(imagpart e)) name gh c))))
  ...
  (cond (name  
	 (multiple-value-bind (val found)(sfind env name) ;; has the name a value?
	   ...
	   (cond (found   ;; it has a value
		 ...
		 #+ignore ;; we want to do something like this, but this is buggy
		 ((or (null plist)(eq (Head e) (car plist))) ; head matches, if necessary..
		  ;; there are now 2 possibilities if gh is flat.
		  ;; Blank can match Default, or Blank can match something that exists.
		  ;; try matching something or nothing..
		  ;;, then if that fails, try matching Default if there is one.
		  (cond ((null e) 
			 (spush env name nil)
			 (cond ((funcall condition) (return-from mblank1 t))(t (spop env) nil))));if it fails, pop it.
		  (cond ((null gh) nil)
			((Default gh) 
			 (spush env name (Default gh))
			 (cond ((funcall condition) (return-from mblank1 t))(t (spop env) nil)))))	
		 )))			;
      ...
      (cond ((atom pk)
	    ...
	    (t  
	     ...
	     ;; (dformat t "~%~s test"(and(not e1) (not(eq (car(third pk)) 'BlankNullSequence))))
	      (cond
	       ((null e1) nil) ; do next combination.
...
'(success ((x . c) (z . gg) (y . d)))
 ...
 (trial '(Plus (Pattern x (Blank)) (Pattern y (Blank)) (Pattern x (Blank)) 	  ) '(Plus a b a))
   ...
   (trial '(Complex (Pattern r (Blank)) (Pattern i (Blank))) (complex 3 4)) '(success ((i . 4) (r . 3)))
...
;; key test 2/4/2011
(m1 '(w (Sin (Pattern x (Blank)))) '(w (Sin a)) nil nil #'truth)   ;; returns t
 (m1 '(w (Sin (Pattern x (Blank)))) '(w (Sin a)) nil 'Plus #'truth) ;; returns t , now

oops, found parsing error: a:_:a:_  should parse the same as we get from (a:_):a:_ .
The latter is syntactically incorrect in mma 7.0
should be (Optional (Pattern a (Blank)) (Pattern a (Blank))) 

For Rubi we need to process patterns like
Int[c_*(a_+b_.*x_),x_Symbol] :=  c*(a+b*x)^2/(2*b) / ; FreeQ[{a,b,c},x]
