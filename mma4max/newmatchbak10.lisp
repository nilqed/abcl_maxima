	 ...
	 (multiple-value-bind (val found)(sfind env name) ;; has the name a value?
	   (cond
	    ...
	    (t ;; it doesn't already have a value
	     (spush env name e) 
	     (format t "~%tentatively assigning  ~s := ~s" name e)
...
(defun mblank2(plist e name  gh condition)
  (format t "~%for mblank2, name=~s, val=~s" name (sfind env name))
  (cond (name  
	 (multiple-value-bind (val found)(sfind env name) ;; has the name a value?
...
(defun mblank3(plist e name  gh condition)
  (format t "~%for mblank3, name=~s, val=~s" name (sfind env name))
  (cond (name  
	 (multiple-value-bind (val found)(sfind env name) ;; has the name a value?
		  ...
		  ;  #+ignore 
		   ...
		   ;; e1
			 ...
			 (let* ((pp (remove pk p :count 1))
				(ee e)
			       (eex (dolist (i e1 ee)(setf ee (remove i ee :count 1)))))
...
(defparameter ftests
  '(
    ...
    ;; work on this one ... currently returns 
    (trial '(g (Pattern x (Blank)) b)  '(g  b a)) '(success ((x . a)))
    (trial '(g a b (Pattern x (BlankSeqence))) '(g a c b )) nil
...
(defparameter tests2
    ...
    ;; now is just '(success((x . a)))  or '(failure)
      ...
      ;; crashed too
      ;;(trial '(Plus (Pattern x1 (BlankSequence)) d e) '(Plus a b c d e))
