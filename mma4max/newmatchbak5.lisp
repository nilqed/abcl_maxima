...
(defun matchtest(pat exp &optional (condition #'truth))
  ...
  (spushframe env 'match)
  (prog1  (m1 pat exp nil nilcondition)
...
(defun m1 (p e name gh c)
  ...
  (let* ((hp (Head p))
	...
	(if (m1 alt e name gh c)
	    (return t)
	  (spopframe env 'Alternatives) ; if this choice failed, try another til exhausted
	  )))
     ((eq hp 'Optional)
...
(defun patfree(p)
  ...
  (cond ((atom p)t) ;; null or atomic has no Sequence
	((member (car p) '(Pattern BlankSequence BlankNullSequence) :test 'eq) nil)))
...
(defun mlist(pl el name gh condition)
  ...
  (cond ((null pl) 
	...
	((patfree (car pl))
	 (if (not (member 'Orderless (Attributes (Head gh)) :test 'eq))
	     ;; no sequences and in order. especially simple
	     (m1 (car pl)(car el) ;; match the first elements
		 name
		 gh
		 #'(lambda()(and (mlist (cdr pl)(cdr el) name  gh condition) ;then the rest
				 (funcall condition))))  
	   ;; gh is ORDERLESS
		...
		(and(equal (meval thedefault)(meval val)) ; meval to be safe?
	       ...
	       (t (cond ((mlist (cons thepat (cdr pl)) el thename gh condition)t)
			(t		;(print '*BB*)
			 ...
			 (cond ((mlist (cdr pl) el nil gh condition)t) ;nil or name?
...
(defun mblank2list (pl el name gh condition)
  ...
  (let ((prevbind nil)
    ...
    (dosublist 
     (ee shorterel 
	 (if (null prevbind)(spop env))) ;; successively, bind ee to () (e1) (e1 e2) ....
	...
	(schange env name (setf tempbind(if (cdr ee)(cons 'Sequence ee) ee)))
  ...
  (let ((pl (cdr p))) ;; list of pattern inside Plus
    ...
    (cond
     ...
     ((null (cdr pl))
      (dformat t "~% singleton ~s to match against ~e" (car pl) e)
       (return-from matchpluspe
			 (m1 (car pl) e nil 'Plus Condition)))
     ...
     ((not(eq (Head e) 'Plus))
      ...
      (dolist (pk pl (funcall Condition)) ;; go through items in pl.
	(dformat t "~%matching pattern ~s in expr ~s" pk e)
	(if (m1 pk e nil 'Plus #'(lambda() (matchpluspe (remove pk p) 0 Condition)))
	    (return t))))
     
     ;; case two:  e IS a Plus.
     (t
      (setf e (cdr e));  remove the Plus
      (dformat t "~% p is a sum of ~s and e is a sum of  ~s" pl e)
      (dolist (pk pl  (funcall Condition)) ;; go through items in pl.
	;; if we have run through all the pattern parts and have
	;; not failed out, then in fact we must have matched all the
	;; parts.
	...
	(if (atom pk)
	    (cond ((find pk e)
		   ...
		   (if (null e) (return (funcall Condition))) nil) ; all done here
		  (t ;;
		   nil			;(return-from matchpluspe nil)
		   )
	 ...
	 (cond((null e1) nil)		;skip empty
	      ...
	      ((m1 pk 
		   ...
		   (let ((pp (remove pk p :count 1))
			 (ee (set-minus e e1)))
		   #'(lambda()(matchpluspe  ;; and the rest matches too,
				      pp
				      (meval (cons 'Plus 
						   ee))
				      Condition))))
	       (return-from matchpluspe  t))
...
(defun runtest(h)
  (if (null h) 'Done
    (if (equal (eval (car h))(cadadr h))(runtest (cddr h))
      (let() (format t "~% problem with ~s expected ~s" (car h)(cadr h))
	  (runtest (cddr h))))))
